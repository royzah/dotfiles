...
> Project instructions for GitHub Copilot in this repository.
...
- **Never** add `Co-authored-by` for AI. Never mention AI in commits or PRs. Human is sole author.
...
## GitHub Copilot Tool Usage

### Core Tools

- Use the available terminal tools to run commands and verify results.
- Read files before editing. Search with `rg` or the available code search tools before creating files.
- Use targeted edits and review the resulting diff.
- Use available web search, documentation, or MCP tools to verify external APIs against official docs.
- Tool availability depends on the client and session. Never claim to have run an unavailable tool.

### Skills and Capabilities (Progressive Disclosure)

- Use relevant skills when available; read their `SKILL.md` before starting.
- For documents, PDFs, presentations, spreadsheets, and UI work, discover available capabilities first.
- Do not assume Claude-specific tools, hooks, plugins, or skills are installed.
- If a required capability is unavailable, state the limitation and use an available alternative.
